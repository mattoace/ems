 ...
 function chooseStream(selVal){
    ...
    var streamDropdown = $('#selector3');
     ...
     $.get("/back/LoopUpDropDowns.php?case=2&sortid="+selVal,function(data){ 
       ...
       fetchAllStudentsStream(institutionDropdown.val(),classDropdown.val(),streamDropdown.val());
      },"json");
...
function fetchAllStudentsStream(instId,classId,StreamId){    
    
  loadUrlAllStudents = "/back/Student_StreamMain.php?classId="+classDropdown.val()+"&Inst="+institutionDropdown.val()+"&streamId="+streamDropdown .val();
     
  tableClass.ajax.url(loadUrlAllStudents).load(); 
...
$(document).ready(function() {
	...
	$('#allStudents').on( 'click', 'tbody td:not(:first-child)', function (e) {
		//editor.inline( this );
	} );

	/*tableClass = $('#allStudents').DataTable( {            
             select: true 
	});
      */
      ...
      tableClass = $('#allStudents').DataTable( {
       ...
       tableClass.select.style( 'multi' );
     	...
     	tableStreams = $('#streamStudents').DataTable( {            
             select: true 
	});
...
function getStudentStreams(){ 
    
            tableStreams.clear();
            
            tableStreams.draw();
            
            tableStreams.select.style( 'multi' );           
            
            $.ajax({
                url: '/back/Student_Streams.php?classId='+classDropdown.val()+ "&inst=" + institutionDropdown.val() + "&streamId="+streamDropdown.val(),
                async: false,
                dataType: 'json',
   ...
   if(rowsSel == 0){
      alert("Please select a student that you want to assign a class!");
       ...
       for(var i=0; i< tableClass.rows('.selected').data().length; i++){           
            ...
            $.get("/back/StudentsRaw.php?case=3&id="+selectedRowsId+"&class_id="+classDropdown.val()+"&institution_id="+institutionDropdown.val()+"&streamId="+streamDropdown.val(),function(data){     
            });
            var rowNode = tableStreams
...
function fromClass(){ 
   ...
   var rowsSel =  tableStreams.rows('.selected').data().length;
   
   if(rowsSel == 0){
      alert("Please select a student that you want to move from a class!");
   }else{  
       var selArray = tableStreams.rows('.selected').data();     
       for(var i=0; i< tableStreams.rows('.selected').data().length; i++){
           var strArr = selArray[i];
                
            var selectedRowsId =strArr[0];
                
            $.get("/back/StudentsRaw.php?case=4&id="+selectedRowsId+"&class_id="+classDropdown.val()+"&institution_id="+institutionDropdown.val()+"&streamId="+streamDropdown.val(),function(data){     
            });          
         }
         
        var rows = tableStreams
        .rows( '.selected' )
        .remove()
        .draw(); 
