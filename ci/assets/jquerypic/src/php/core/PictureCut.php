...
$conn = mysql_connect("localhost","coreictc_ems","coreictc_ems2016");//or die(mysql_error());
mysql_select_db("coreictc_ems");
class PictureCut{
				...
				if (move_uploaded_file($this->fileStream['tmp_name'], $this->currentFile)){
					...
					if($this->enableResize == "true"){
						...
						if(($this->currentWidth > $this->minimumWidthToResize) || ($this->currentHeight > $this->minimumHeightToResize))
						{
                            ...
                            $filepath = "http://localhost/ems/ci/assets/uploads/".$this->currentFileName;
                            $filepath = "http://ems.coreict.co.ke/assets/uploads/".$this->currentFileName;
							$curl = curl_init();
                            ...
                            $sql = "UPDATE ems_person SET img = '".$imageData ."' , img_name = '".$this->currentFileName."' WHERE id = '".$_POST["id"]."'";
	...
	public function crop($student_id){
		try
		{			
				$this->TVimageManipulation = new TVimageManipulation($this->currentFile);
