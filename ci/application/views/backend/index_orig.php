...
<script src="<?php echo base_url("assets/lightstreamer/imChat.js");?>"></script>
 ...
 <body class="sticky-header left-side-collapsed" ><!--initMap()-->
    <section>
    <!-- left side start-->
			...
			<div class="left-side-inner">
				...
				<!--sidebar nav start-->
					<ul class="nav nav-pills nav-stacked custom-nav">
							...
							<a href="#"><i class="lnr lnr-cog"></i>
								...
								<ul class="sub-menu-list">
									...
									<li><a href="<?php echo base_url("index.php/students");?>">Students</a></li>
                                               ...
                                               <?php 
                                               $query = $this->db->query('SELECT id,menu_name,class FROM ems_menu WHERE is_active = 1');                                                        
                                                            foreach ($query->result() as $row)
                                                                {  
                                                                  ...
                                                                  print('<ul class="sub-menu-list">');
                                                                        ...
                                                                        $querySub = $this->db->query('SELECT id,menu_name,url FROM ems_submenu WHERE menu_id = '.$row->id);
                                                                               foreach ($querySub->result() as $rowSub)
                                                                                { 
                                                                                 print('<li><a href='.base_url(''.$rowSub->url.'').'>'.$rowSub->menu_name.'</a> </li>'); 
                                                                                }     
						...
						<!--<li class="menu-list"><a href="#"><i class="lnr lnr-book"></i>  <span>Others</span></a> 
							<ul class="sub-menu-list">
							...
							<li class="dropdown profile_details_drop">
								<a href="#" class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
									<div class="profile_img">                                                                            
                                                                               <?php
                                                                                $query = $this->db->query('SELECT id,image,image_name FROM ems_imgtmp');                                                        
                                                                                $row = $query->result();
                                                                                 print("<span ><img style='border-radius: 25px' height='40' width='40' src='".$row[0]->image."'  class='preview'/></span>");
                                                                                $query = $this->db->query('SELECT fname FROM ems_users');                                                        
                                                                                $row = $query->result(); 
                                                                                ?>								
                                                                                <div class="user-name">
